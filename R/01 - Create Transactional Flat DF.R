...
create_flat_trans <- function(.full_trans) {
  ...
  # paste(seq(from = 813.00, to = 813.99, by = 0.01), collapse = "|") #diagnosis for forearm fx
  ...
  .flat_trans <- .full_trans %>%
    ...
    mutate.(fltr_diagnosis = str_detect(code_cd_diagnosis, #diagnosis for forearm fx
                                  pattern = paste(seq(from = 813.00, to = 813.99, by = 0.01), collapse = "|")),
            fltr_procedure = str_detect(code_cd_procedure,
                                  pattern = list('79.02', '79.12', '79.22', '79.32')),#forearm surgery
